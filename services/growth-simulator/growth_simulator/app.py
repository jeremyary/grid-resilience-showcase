...
async def lifespan(app: FastAPI) -> AsyncIterator[None]:
    ...
    predictor = ComputationalPredictor(
...
def _load_grid_data() -> dict[str, Any]:
    ...
    with psycopg.connect(settings.dsn) as conn:
        with conn.cursor() as cur:
            cur.execute(
                "SELECT id, name, normal_capacity_mw, emergency_capacity_mw, "
            ...
            cur.execute(
                "SELECT id, asset_type, feeder_id, rated_kva, customers_downstream, "
                "lat, lon FROM assets WHERE asset_type = 'transformer' AND status = 'in_service'"
            )
            cols = [d[0] for d in (cur.description or [])]
            transformers = [dict(zip(cols, row, strict=False)) for row in cur.fetchall()]

            cur.execute(
                "SELECT id, feeder_id, customers_downstream FROM assets "
                "WHERE status = 'in_service'"
            )
            cols = [d[0] for d in (cur.description or [])]
            all_assets = [dict(zip(cols, row, strict=False)) for row in cur.fetchall()]

    return {"feeders": feeders, "transformers": transformers, "all_assets": all_assets}
